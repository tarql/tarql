...
## Delimiters, quotes and character encoding in CSV/TSV files

The following syntax options can be adjusted:

- Delimiter character (the character used to separate fields in a row; typically comma, tab, or semicolon)
- Quote character (the character used to enclose field values where the delimiter or a line break occurs within the value; typically, double or single quote)
- Escape character (the character used to escape quotes occurring within quoted values; typically none or backslash)
...
1. On the command line (`--delimiter`, `--tabs`, `--quotechar`, `--escapechar`, `--encoding`, `--header`, `--no-header`)
2. By appending a pseudo-fragment to the CSV file's URL in the `FROM` clause or on the command line (e.g., `file.csv#delimiter=tab`, `file.csv#delimiter=%3B`, `file.csv#quotechar=singlequote;escapechar=backslash`, `file.csv#encoding=latin1`, `file.csv#header=absent`)   
