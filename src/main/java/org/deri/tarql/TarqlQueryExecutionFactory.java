...
public class TarqlQueryExecutionFactory {
	...
	public static TarqlQueryExecution create(TarqlQuery query) throws IOException {
		return create(query, StreamManager.get(), new CSVOptions());
	}

	public static TarqlQueryExecution create(TarqlQuery query, CSVOptions options) throws IOException {
		return create(query, StreamManager.get(), options);
	}

	public static TarqlQueryExecution create(TarqlQuery query, StreamManager fm) throws IOException {
	...
	public static TarqlQueryExecution create(TarqlQuery query, StreamManager fm, CSVOptions options) throws IOException {
		String filenameOrURL = getSingleFromClause(query.getQueries().get(0), fm);
		URLOptionsParser parseResult = new URLOptionsParser(filenameOrURL);
	...
	private static String getSingleFromClause(Query query, StreamManager fm) {
